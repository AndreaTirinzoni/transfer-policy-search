...
disc = np.zeros((runs, num_batch))
policy = np.zeros((runs, num_batch, param_space_size))
...
for i_run in range(runs):
    ...
    # print("PD-IS")
    # estimator = "PD-IS"
    # pff_policy_pd_is = la.learnPolicy(env_param, simulation_param, source_dataset, estimator, off_policy=1) #1e-6
    #
    print("MIS")
    estimator = "MIS"
...
# mean_pol3 = np.mean(disc, axis=0)
# #var_pol1 = np.std(off_policy_mis_cv_baseline.disc_rewards, axis=0) / (m.sqrt(runs))
# var_pol3 = np.std(disc, axis=0) / (m.sqrt(runs))
#
# plot.plot_curves([x], [mean_pol3], [var_pol3], title="Discounted rewards over batches", x_label="Batch", y_label="Rewards", names=["MIS-CV-BASELINE", "G(PO)MDP"])
#
# #mean_pol1 = np.mean(off_policy_mis_cv_baseline.policy_parameter, axis=0)
# mean_pol3 = np.mean(policy, axis=0)
# #var_pol1 = np.std(off_policy_mis_cv_baseline.policy_parameter, axis=0) / (m.sqrt(runs))
# var_pol3 = np.std(policy, axis=0) / (m.sqrt(runs))
#
#
# for i in range(param_space_size):
#     plot.plot_curves([x], [mean_pol3[:, i]], [var_pol3[:, i]], title = "Policy parameter over batches", x_label = "Batch", y_label = "Policy parameter", names=["MIS-CV-BASELINE", "G(PO)MDP"])
